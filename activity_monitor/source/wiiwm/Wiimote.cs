...
namespace WiimoteLib
{
	...
	/// </summary>
	public class WiimoteReadEventArgs: EventArgs
	{
		/// <summary>
		...
		/// Constructor
		/// </summary>
		/// <param name="buffer">Wiimote state</param>
		public WiimoteReadEventArgs(byte[] buffer)
		{
			buff = buffer;
		}
	}
	...
	{
        protected event EventHandler<WiimoteReadEventArgs> WiimoteDataRead;
            ...
            try
            {
                while (mStream.CanRead)
                {
                    ...
                    while (offset < REPORT_LENGTH)
                        offset += mStream.Read(buff, offset, REPORT_LENGTH - offset);

                    WiimoteDataRead(this, new WiimoteReadEventArgs(buff));
                }
            }
            catch (IOException e) { }
        }
		...
		private void OpenWiimoteDeviceHandle(string devicePath)
		{
			...
			mStream = new FileStream("WWM0:", FileMode.Open, FileAccess.ReadWrite);
		...
		/// </summary>
		private void BeginSyncRead()
		{
			// if the stream is valid and ready
            if (mStream != null && mStream.CanRead)
			{
				//WiimoteStream.stream = mStream;
                //WiimoteStream.bufLen = REPORT_LENGTH;
                WiimoteDataRead += OnReadData;
                wiiReadThread = new Thread(new ThreadStart(SyncRead));
                wiiReadThread.Start();
		...
		private void OnReadData(object sender, WiimoteReadEventArgs eArgs)
		{
			...
			try
			{
				...
				// parse it
				if(ParseInputReport(eArgs.buff))
				{
					// post an event
